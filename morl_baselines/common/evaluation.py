...
def _to_list(x):
    return x.tolist() if hasattr(x, "tolist") else list(x) if hasattr(x, "__iter__") else [float(x)]
...
def log_all_multi_policy_metrics(
    ...
    logger.info("Evaluation: HV=%.4f, EUM=%.4f, Cardinality=%d, global_step=%d",
                hv, eum, card, global_step,
                extra={"json_event": {
                    "event": "evaluation",
                    "global_step": global_step,
...
def log_episode_info(
    ...
    if verbose:
        logger.info("Episode infos: Steps=%d, Time=%s, Return=%s, DiscReturn=%s, "
                    "ScalarReturn=%s, DiscScalar=%s",
                    episode_ts, episode_time, episode_return, disc_episode_return,
                    scal_return, disc_scal_return,
                    extra={"json_event": {
                        "event": "episode",
                        "timesteps": int(episode_ts),
                        "episode_time": episode_time,
                        "return": _to_list(episode_return),
                        "discounted_return": _to_list(disc_episode_return),
                        "scalarized_return": float(scal_return),
                        "discounted_scalarized_return": float(disc_scal_return),
