...
class MOAgent(ABC):
    ...
    def extract_env_info(self, env: Optional[gym.Env]) -> None:
        ...
        if env is not None:
            self.env = env
            is_vector = isinstance(env, VectorEnv)
            obs_space = env.single_observation_space if is_vector else env.observation_space
            act_space = env.single_action_space if is_vector else env.action_space
            ...
            if isinstance(obs_space, spaces.Discrete):
                ...
                self.observation_dim = obs_space.shape[0]

            self.action_space = act_space
            if isinstance(act_space, (spaces.Discrete, spaces.MultiBinary)):
                self.action_shape = (1,)
