...
class GPIPDContinuousAction(MOAgent, MOPolicy):
    ...
    def update(self, weight: th.Tensor):
        ...
        if self.global_step % 100 == 0:
            ...
            logger.info("Training: critic_loss=%.4f policy_loss=%.4f global_step=%d",
                        critic_loss.item(), policy_loss.item(), self.global_step,
                        extra={"json_event": {
    ...
    def train_iteration(
        ...
        is_vector = isinstance(self.env, VectorEnv)
        num_envs = self.env.num_envs if is_vector else 1

        self.global_step = 0 if reset_num_timesteps else self.global_step
        self.num_episodes = 0 if reset_num_timesteps else self.num_episodes

        obs, info = self.env.reset()
        ep_returns = np.zeros((num_envs, self.reward_dim))

        for _ in range(1, total_timesteps // num_envs + 1):
            self.global_step += num_envs

            # --- action selection (batched for vector env) ---
            if self.global_step < self.learning_starts:
                if is_vector:
                    actions = np.array([self.action_space.sample() for _ in range(num_envs)])
                else:
                ...
                with th.no_grad():
                    ...
                    if is_vector:
                        w_t = tensor_w.repeat(num_envs, 1)
                    else:
                        w_t = tensor_w
                    actions = (
                        self.policy(
            ...
            # --- step ---
            next_obs, vector_rewards, terminateds, truncateds, infos = self.env.step(actions)

            # --- buffer insert ---
            if is_vector:
                for i in range(num_envs):
                    self.replay_buffer.add(
                        obs[i], actions[i], vector_rewards[i],
                        next_obs[i], terminateds[i] or truncateds[i],
                    )
                    ep_returns[i] += vector_rewards[i]
            else:
                self.replay_buffer.add(obs, actions, vector_rewards, next_obs, terminateds or truncateds)
            ...
            if is_vector:
                ...
                if terminateds or truncateds:
                    ...
                    if "episode" in infos.keys():
                        log_episode_info(infos["episode"], np.dot, weight, self.global_step, verbose=True)
                    if change_weight_every_episode:
                        weight = random.choice(weight_support)
                        tensor_w = th.tensor(weight).float().to(self.device)
                else:
    ...
    def train(
        ...
        Args:
            ...
            eval_env_fn (callable, optional): Factory returning a fresh env instance. When provided with num_eval_workers > 1, multi-weight evaluation runs in parallel via ThreadPoolExecutor.
            num_eval_workers (int): Number of parallel workers for multi-weight evaluation. Default 1 (sequential).
        """
        if self.log:
        ...
        for iter in range(1, max_iter + 1):
            ...
            logger.info("Next weight vector: %s", w,
                        extra={"json_event": {
                            "event": "weight_vector",
                            "weight": w.tolist(),
                            "iteration": iter,
            ...
            if weight_selection_algo == "ols":
                ...
                if num_eval_workers > 1 and eval_env_fn is not None:
                    from concurrent.futures import ThreadPoolExecutor
                    def _eval_corner(wcw):
                        env = eval_env_fn()
                        try:
                            return policy_evaluation_mo(self, env, wcw, rep=num_eval_episodes_for_front)[3], wcw
                        finally:
                            env.close()
                    with ThreadPoolExecutor(max_workers=min(num_eval_workers, len(M))) as pool:
                        results = list(pool.map(_eval_corner, M))
                    for n_value, wcw in results:
            ...
            if self.global_step % eval_mo_freq == 0:
                # Evaluation
                if num_eval_workers > 1 and eval_env_fn is not None:
                    from concurrent.futures import ThreadPoolExecutor
                    def _eval_one(weight):
                        env = eval_env_fn()
                        try:
                            return policy_evaluation_mo(self, env, weight, rep=num_eval_episodes_for_front)[3]
                        finally:
                            env.close()
                    with ThreadPoolExecutor(max_workers=num_eval_workers) as pool:
                        gpi_returns_test_tasks = list(pool.map(_eval_one, eval_weights))
                else:
