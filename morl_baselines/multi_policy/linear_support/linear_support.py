...
class LinearSupport:
    ...
    def next_weight(
        ...
        if len(self.ccs) > 0:
            ...
            if self.verbose:
                logger.info("W_corner: %s W_corner size: %d", W_corner, len(W_corner))

            self.queue = []
            if algo == "gpi-ls":
                if gpi_agent is None:
                    raise ValueError("GPI-LS requires passing a GPI agent.")
                gpi_expanded_set = [policy_evaluation_mo(gpi_agent, env, w, rep=rep_eval)[3] for w in W_corner]
            for wc in W_corner:
                if algo == "ols":
        ...
        if self.verbose:
            logger.info("CCS: %s CCS size: %d", self.ccs, len(self.ccs))
        ...
        if len(self.queue) == 0:
            ...
            if self.verbose:
                logger.info("Next weight: %s", next_w,
                            extra={"json_event": {
                                "event": "next_weight",
                                "weight": _to_list(next_w),
                            }})
            return next_w
    ...
    def add_solution(self, value: np.ndarray, w: np.ndarray) -> List[int]:
        ...
        """
        if self.verbose:
            logger.info("Adding value=%s for weight=%s to CCS.", value, w,
                        extra={"json_event": {
                            "event": "add_solution",
        ...
        if self.is_dominated(value):
            if self.verbose:
                logger.info("Value %s is dominated. Discarding.", value)
            return [len(self.ccs)]
    ...
    def remove_obsolete_values(self, value: np.ndarray) -> List[int]:
        ...
        for i in reversed(range(len(self.ccs))):
            ...
            if len(weights_optimal) == 0:
                logger.info("removed value %s", self.ccs[i])
    ...
    def max_value_lp(self, w_new: np.ndarray) -> float:
        ...
        try:
            ...
            logger.warning("ECOS solver error, trying SCS.")
